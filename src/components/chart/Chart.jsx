...
const generateChartData = (columnIndex) => {
  ...
  csvData.slice(1).forEach(row => {
    ...
    if (!uniqueValues.has(value)) {
      ...
      const existingIndex = chartData.findIndex(([existingValue]) => existingValue === value);
  ...
  return (
   ...
   <div className="chart-container">
    {csvData.length > 0 && (
        <div >
           {csvData[0].slice(1,csvData.length-2).map((columnName, columnIndex) => { // Skip the first column and its header
            const { chartData, colors } = generateChartData(columnIndex + 1); // Adjust column index
            return (
